...
for PAIR in $TEST_PAIRS; do
    ...
    # 1. COMPUTE BOUND
    srun --nodes=$N --ntasks=$P --ntasks-per-node=$P_PER_NODE ./mpi_overlap $SCENARIO_1_NAME $SCENARIO_1_WORK $SCENARIO_1_MSG $N
    # 2. LATENCY BOUND
    srun --nodes=$N --ntasks=$P --ntasks-per-node=$P_PER_NODE ./mpi_overlap $SCENARIO_2_NAME $SCENARIO_2_WORK $SCENARIO_2_MSG $N
    # 3. BANDWIDTH BOUND
    srun --nodes=$N --ntasks=$P --ntasks-per-node=$P_PER_NODE ./mpi_overlap $SCENARIO_3_NAME $SCENARIO_3_WORK $SCENARIO_3_MSG $N
    # 4. MAX BANDWIDTH BOUND
    srun --nodes=$N --ntasks=$P --ntasks-per-node=$P_PER_NODE ./mpi_overlap $SCENARIO_4_NAME $SCENARIO_4_WORK $SCENARIO_4_MSG $N

done

echo "Done. See 7_results.csv"